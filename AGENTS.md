...
- Structure of a multi-module project:
  ...
  - `main.gom` - the entry point source file
  - `PackageName/goml.toml` - sub-package configuration with `[package]` section
  - `PackageName/lib.gom` - library package source files
  - `main.gom.out` - expected execution output
- Package naming conventions:
  - Source files no longer declare `package`; package identity comes from `goml.toml` or the compiler command
  - The main package contains the `fn main()` entry point
...
- To add a new multi-module test:
  ...
  3. Create `main.gom` with `fn main()`
