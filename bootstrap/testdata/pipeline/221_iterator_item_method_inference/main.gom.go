...
func main0() struct{} {
    var t72 Values = Values{}
    var for_iter64 Values = _goml_m_trait__impl_i_IntoIterator_i_Values_i_into__iter(t72)
    Loop_loop74:
    for {
        if true {
            var for_next65 Option__int32 = _goml_m_trait__impl_i_Iterator_i_Values_i_next(for_iter64)
            switch for_next65.(type) {
            case None:
                break Loop_loop74
            case Some:
                var x66 int32 = for_next65.(Some)._0
                var value__1 int32 = x66
