...
pub fn encode_go(path: &Path, source: &str) -> String {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || encode_go_inner(&path, &source))
        .unwrap()
        .join()
        .unwrap()
}

fn encode_go_inner(path: &Path, source: &str) -> String {
    use cst::cst::CstNode;
    ...
    let (anf, anfenv) = compiler::anf::anf_file(liftenv, &gensym, lift);
    let (go, goenv) = compiler::go::compile::go_file(anfenv, &gensym, anf);
    go.to_pretty(&goenv, 120)
}

pub fn encode_anf(path: &Path, source: &str) -> String {
    let path = path.to_owned();
    let source = source.to_owned();
    std::thread::Builder::new()
        .stack_size(64 * 1024 * 1024)
        .spawn(move || encode_anf_inner(&path, &source))
        .unwrap()
        .join()
        .unwrap()
}

fn encode_anf_inner(path: &Path, source: &str) -> String {
    use cst::cst::CstNode;
    use parser::syntax::MySyntaxNode;
