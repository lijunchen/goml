...
diff_lines=${GOMLANG_MONO_DIFF_LINES:-0}
...
while IFS= read -r -d '' source; do
  if ! "$oracle" mono "$source" >"$work_dir/rust"; then
    printf 'Rust Mono oracle failed: %s\n' "$source" >&2
    exit 1
  fi
  ...
  if ! cmp --silent "$work_dir/rust" "$work_dir/goml"; then
    printf 'Mono mismatch after %d matched files: %s\n' "$count" "$source" >&2
...
printf 'matched %d GoML Mono corpus files\n' "$count"
