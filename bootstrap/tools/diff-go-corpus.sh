...
diff_lines=${GOMLANG_GO_DIFF_LINES:-0}
...
while IFS= read -r -d '' source; do
  if ! "$oracle" go "$source" >"$work_dir/rust"; then
    printf 'Rust Go oracle failed: %s\n' "$source" >&2
    exit 1
  fi
  if [[ ! -s "$work_dir/rust" ]]; then
    continue
  fi
  if ! "$parser" __canonical-stage go "$source" >"$work_dir/goml"; then
    printf 'GoML Go lowering failed: %s\n' "$source" >&2
    exit 1
  fi
  if ! cmp --silent "$work_dir/rust" "$work_dir/goml"; then
    printf 'Go mismatch after %d matched files: %s\n' "$count" "$source" >&2
...
printf 'matched %d GoML Go corpus files\n' "$count"
