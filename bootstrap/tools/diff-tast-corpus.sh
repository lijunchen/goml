...
diff_lines=${GOMLANG_TAST_DIFF_LINES:-0}
...
while IFS= read -r -d '' source; do
  if ! "$oracle" tast "$source" >"$work_dir/rust"; then
    printf 'Rust TAST oracle failed: %s\n' "$source" >&2
    exit 1
  fi
  if [[ ! -s "$work_dir/rust" ]]; then
    continue
  fi
  if ! "$parser" __canonical-stage tast "$source" >"$work_dir/goml"; then
    printf 'GoML TAST checking failed: %s\n' "$source" >&2
    exit 1
  fi
  if ! cmp --silent "$work_dir/rust" "$work_dir/goml"; then
    printf 'TAST mismatch after %d matched files: %s\n' "$count" "$source" >&2
...
printf 'matched %d GoML TAST corpus files\n' "$count"
