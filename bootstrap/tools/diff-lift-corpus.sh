...
diff_lines=${GOMLANG_LIFT_DIFF_LINES:-0}
...
while IFS= read -r -d '' source; do
  if ! "$oracle" lift "$source" >"$work_dir/rust"; then
    printf 'Rust Lift oracle failed: %s\n' "$source" >&2
    exit 1
  fi
  ...
  if ! cmp --silent "$work_dir/rust" "$work_dir/goml"; then
    printf 'Lift mismatch after %d matched files: %s\n' "$count" "$source" >&2
...
printf 'matched %d GoML Lift corpus files\n' "$count"
