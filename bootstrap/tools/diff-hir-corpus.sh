...
while IFS= read -r -d '' source; do
  if ! "$oracle" hir "$source" >"$work_dir/rust"; then
    printf 'Rust HIR oracle failed: %s\n' "$source" >&2
    exit 1
  fi
  if [[ ! -s "$work_dir/rust" ]]; then
    continue
  fi
  if ! "$parser" __canonical-stage hir "$source" >"$work_dir/goml"; then
    printf 'GoML HIR lowering failed: %s\n' "$source" >&2
    exit 1
  fi
  if ! cmp --silent "$work_dir/rust" "$work_dir/goml"; then
    printf 'HIR mismatch: %s\n' "$source" >&2
...
printf 'matched %d GoML HIR corpus files\n' "$count"
