...
const loadDemos = async () => {
  const modules = import.meta.glob('../../crates/compiler/src/tests/pipeline/*/main.gom', {
    query: '?raw',
    import: 'default'
  });
  ...
  const loadedEntries = await Promise.all(
    Object.entries(modules).map(async ([path, loader]) => {
      // Extract folder name from path like "../../crates/compiler/src/tests/pipeline/000/main.gom"
  ...
  useEffect(() => {
    if (monaco) {
      ...
      monaco.languages.setMonarchTokensProvider('simple', {
        ...
        tokenizer: {
          root: [
            [/\b(fn|struct|enum|trait|impl|for|match|if|else|let|in|return|true|false|unit|bool|int8|int16|int32|int64|uint8|uint16|uint32|uint64|string)\b/, "keyword"],
