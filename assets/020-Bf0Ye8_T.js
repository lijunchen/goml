...
fn point32_to_string(point: Point) -> string {
    ...
    let with_x = string_add("Point { x: ", int32_to_string(x));
    let with_y_label = string_add(with_x, ", y: ");
    let with_y = string_add(with_y_label, int32_to_string(y));
    string_add(with_y, " }")
}

fn wrapper_int32_to_string(wrapper: Wrapper[int32]) -> string {
    let Wrapper { value: value } = wrapper;
    let prefix = string_add("Wrapper[int32] { value: ", int32_to_string(value));
    string_add(prefix, " }")
}

fn wrapper_unit_to_string(wrapper: Wrapper[unit]) -> string {
    let Wrapper { value: value } = wrapper;
    let prefix = string_add("Wrapper[unit] { value: ", unit_to_string(value));
    string_add(prefix, " }")
}
...
fn shape_int32_to_string(shape: Shape[int32]) -> string {
    match shape {
        Dot(point) => {
            let prefix = string_add("Shape::Dot(", point32_to_string(point));
            string_add(prefix, ")")
        },
        Wrapped(wrapper) => {
            let prefix = string_add("Shape::Wrapped(", wrapper_int32_to_string(wrapper));
            string_add(prefix, ")")
        },
        Origin => "Shape::Origin",
    }
...
fn shape_unit_to_string(shape: Shape[unit]) -> string {
    match shape {
        ...
        Wrapped(wrapper) => {
            let prefix = string_add("Shape::Wrapped(", wrapper_unit_to_string(wrapper));
            string_add(prefix, ")")
        },
        Origin => "Shape::Origin",
    }
