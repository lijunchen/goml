...
fn build_counter(label: string, start: int32) -> (Tracker, () -> Record[int32], (int32) -> Record[int32], () -> Record[string]) {
    let count = Ref::new(start);
    let toggled = Ref::new(false);
    ...
    let bump = |delta: int32| {
        ...
        let _ = count.set(before + delta);
        Pair(before, count.get())
    };
    let flip = || {
        let before = toggled.get();
        let _ = toggled.set(!before);
        let after = toggled.get();
