...
fn main() {
    ...
    let printer = |prefix: string, value: int32| {
        let message = string_add(prefix, int32_to_string(value));
    ...
    let play_list_and_point = || {
        match list123 {
            ...
            Cons(head, tail) => {
                ...
                match point {
                    Point { x: x, y: y } => {
                        let _ = string_println(string_add("Point: (", string_add(int32_to_string(x), string_add(", ", string_add(int32_to_string(y), ")")))));
                        ()
                    }
                }
            }
