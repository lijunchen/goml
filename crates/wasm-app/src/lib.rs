...
pub fn compile_to_mono(src: &str) -> String {
    ...
    let (mono, monoenv) = match compiler::mono::mono(genv, core) {
        Ok(result) => result,
        Err(message) => return format!("error (compile): {}", message),
    };
...
pub fn colon_colon_completions(src: &str, line: u32, col: u32) -> String {
    ...
    for item in items {
        ...
        let kind_str = match kind {
            compiler::query::ColonColonCompletionKind::Namespace => "namespace",
...
pub fn value_completions(src: &str, line: u32, col: u32) -> String {
    ...
    for item in items {
        ...
        let kind_str = match kind {
            ...
            compiler::query::ValueCompletionKind::Namespace => "namespace",
