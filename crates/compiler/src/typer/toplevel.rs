...
fn define_enum(genv: &mut GlobalTypeEnv, enum_def: &ast::EnumDef) {
    ...
    let variants = enum_def
        ...
        .map(|(vcon, typs)| {
            let typs = typs
                .iter()
                .map(|ast_ty| tast::Ty::from_ast_with_tparams_env(ast_ty, params_env))
                .collect::<Vec<_>>();
            for ty in typs.iter() {
...
fn define_struct(genv: &mut GlobalTypeEnv, struct_def: &ast::StructDef) {
    ...
    let fields = struct_def
        ...
        .iter()
        .map(|(fname, ast_ty)| {
            let ty = tast::Ty::from_ast_with_tparams_env(ast_ty, &struct_def.generics);
            validate_ty(genv, &ty, &tparam_names);
            (fname.clone(), ty)
        })
        .collect();
...
fn define_trait(genv: &mut GlobalTypeEnv, trait_def: &ast::TraitDef) {
    ...
    } in trait_def.method_sigs.iter()
    {
        ...
        let param_tys = params
            .iter()
            .map(|ast_ty| tast::Ty::from_ast_with_tparams_env(ast_ty, &[]))
            .collect::<Vec<_>>();
        let ret_ty = tast::Ty::from_ast_with_tparams_env(ret_ty, &[]);
...
fn define_impl(genv: &mut GlobalTypeEnv, impl_block: &ast::ImplBlock) {
    ...
    let for_ty = tast::Ty::from_ast_with_tparams_env(&impl_block.for_type, &[]);
    ...
    for m in impl_block.methods.iter() {
        ...
        let params = m
            ...
            .iter()
            .map(|(_, ty)| {
                let ty = tast::Ty::from_ast_with_tparams_env(ty, &m.generics);
                validate_ty(genv, &ty, &tparam_names);
                ty
            })
            .collect::<Vec<_>>();
        let ret = match &m.ret_ty {
            Some(ty) => {
                let ret = tast::Ty::from_ast_with_tparams_env(ty, &m.generics);
...
fn define_function(genv: &mut GlobalTypeEnv, func: &ast::Fn) {
    ...
    let params = func
        ...
        .iter()
        .map(|(_, ty)| {
            let ty = tast::Ty::from_ast_with_tparams_env(ty, &func.generics);
            validate_ty(genv, &ty, &tparam_names);
            ty
        })
        .collect::<Vec<_>>();
    let ret = match &func.ret_ty {
        Some(ty) => {
            let ret = tast::Ty::from_ast_with_tparams_env(ty, &func.generics);
            validate_ty(genv, &ret, &tparam_names);
            ret
        }
        None => tast::Ty::TUnit,
...
fn define_extern_go(genv: &mut GlobalTypeEnv, ext: &ast::ExternGo) {
    let params = ext
        ...
        .iter()
        .map(|(_, ty)| {
            let ty = tast::Ty::from_ast_with_tparams_env(ty, &[]);
    ...
    let ret = match &ext.ret_ty {
        Some(ty) => {
            let ret = tast::Ty::from_ast_with_tparams_env(ty, &[]);
...
fn check_fn(genv: &GlobalTypeEnv, typer: &mut Typer, f: &ast::Fn) -> tast::Fn {
    ...
    let param_types: Vec<(Lident, tast::Ty)> = f
        ...
        .map(|(name, ty)| {
            (
                name.clone(),
                tast::Ty::from_ast_with_tparams_env(ty, &f.generics),
            )
        })
        .collect();
    ...
    let ret_ty = match &f.ret_ty {
        Some(ty) => tast::Ty::from_ast_with_tparams_env(ty, &f.generics),
        None => tast::Ty::TUnit,
    };
...
fn check_extern_go(
    ...
    let params = ext
        .params
        .iter()
        .map(|(name, ty)| (name.0.clone(), tast::Ty::from_ast_with_tparams_env(ty, &[])))
        .collect::<Vec<_>>();
    let ret_ty = match &ext.ret_ty {
        Some(ty) => tast::Ty::from_ast_with_tparams_env(ty, &[]),
        None => tast::Ty::TUnit,
    };
    tast::ExternGo {
