...
pub fn check_file(ast: ast::File) -> (tast::File, env::Env) {
    ...
    toplevel::collect_typedefs(&mut env, &ast);
...
fn check_fn(env: &mut Env, typer: &mut TypeInference, f: &ast::Fn) -> tast::Fn {
    ...
    let typed_body = typer.with_tparams_env(&f.generics, |typer| typer.infer(env, &vars, &f.body));
    typer.solve(env);
...
fn check_impl_block(
    ...
    for f in impl_block.methods.iter() {
        ...
        let typed_body =
            typer.with_tparams_env(&f.generics, |typer| typer.infer(env, &vars, &f.body));
        typer.solve(env);
        let typed_body = typer.subst(env, typed_body);
