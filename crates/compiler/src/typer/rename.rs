...
#[derive(Debug)]
struct Env(im::Vector<(ast::Ident, fir::Ident)>);
...
impl Rename {
    ...
    pub fn rename_file(&self, ast: ast::File) -> fir::File {
    ...
    pub fn rename_item(&self, item: &ast::Item, global_funcs: &HashSet<String>) -> fir::Item {
        match item {
            ...
            ast::Item::Fn(func) => fir::Item::Fn(self.rename_fn(func, global_funcs)),
    ...
    pub fn rename_fn(&self, func: &ast::Fn, global_funcs: &HashSet<String>) -> fir::Fn {
        let ast::Fn {
            attrs,
            name,
            generics,
        ...
        match expr {
            ...
            ast::Expr::EStructLiteral { name, fields } => fir::Expr::EStructLiteral {
                ...
                fields: fields
                    .iter()
                    .map(|(field_name, expr)| {
                        (field_name.into(), self.rename_expr(expr, env, global_funcs))
                    })
                    .collect(),
            },
    ...
    fn rename_pat(&self, pat: &ast::Pat, env: &mut Env) -> fir::Pat {
        match pat {
            ...
            ast::Pat::PStruct { name, fields } => {
                let new_fields = fields
                    .iter()
                    .map(|(fname, pat)| (fname.into(), self.rename_pat(pat, env)))
                    .collect();
                fir::Pat::PStruct {
                    name: name.into(),
                    fields: new_fields,
...
impl Rename {
    fn rename_closure_param(&self, param: &ast::ClosureParam, env: &mut Env) -> fir::ClosureParam {
