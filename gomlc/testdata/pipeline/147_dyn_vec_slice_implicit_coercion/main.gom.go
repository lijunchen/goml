package main

import (
    _goml_fmt "fmt"
)

func _goml_runtime_core_int32_to_string(x int32) string {
    return _goml_fmt.Sprintf("%d", x)
}

func _goml_runtime_core_string_println(s string) struct{} {
    _goml_fmt.Println(s)
    return struct{}{}
}

type _goml_vec_Dyn_Display struct {
    items []dyn__Display
}

func vec_new__Vec_11Dyn_Display() *_goml_vec_Dyn_Display {
    return &_goml_vec_Dyn_Display{
        items: nil,
    }
}

func vec_push__Vec_11Dyn_Display(vec *_goml_vec_Dyn_Display, elem dyn__Display) struct{} {
    vec.items = append(vec.items, elem)
    return struct{}{}
}

type dyn__Display_vtable struct {
    show func(any) string
}

type dyn__Display struct {
    data any
    vtable *dyn__Display_vtable
}

func dyn__Display__wrap__int32__show(self any) string {
    return _goml_m_trait__impl_i_Display_i_int32_i_show(self.(int32))
}

func dyn__Display__vtable__int32() *dyn__Display_vtable {
    return &dyn__Display_vtable{
        show: dyn__Display__wrap__int32__show,
    }
}

func _goml_m_trait__impl_i_Display_i_int32_i_show(self__0 int32) string {
    var inline214 string = _goml_runtime_core_int32_to_string(self__0)
    return inline214
}

func main0() struct{} {
    var v__2 *_goml_vec_Dyn_Display
    var inline238 *_goml_vec_Dyn_Display = vec_new__Vec_11Dyn_Display()
    v__2 = inline238
    var first__3 int32 = 10
    var second__4 int32 = 20
    var t188 dyn__Display = dyn__Display{
        data: int32(first__3),
        vtable: dyn__Display__vtable__int32(),
    }
    vec_push__Vec_11Dyn_Display(v__2, t188)
    var t189 dyn__Display = dyn__Display{
        data: int32(second__4),
        vtable: dyn__Display__vtable__int32(),
    }
    vec_push__Vec_11Dyn_Display(v__2, t189)
    var s__5 []dyn__Display
    var inline230 int = 0
    var inline231 int = 2
    var inline232 []dyn__Display = v__2.items[inline230:inline231]
    s__5 = inline232
    var t190 dyn__Display = s__5[0]
    var t191 string
    var inline228 string = t190.vtable.show(t190.data)
    t191 = inline228
    var inline225 string = _goml_m_trait__impl_i_ToString_i_string_i_to__string(t191)
    _goml_runtime_core_string_println(inline225)
    var t__6 []dyn__Display
    var inline221 int = 1
    var inline222 int = 2
    var inline223 []dyn__Display = s__5[inline221:inline222]
    t__6 = inline223
    var t192 dyn__Display = t__6[0]
    var t193 string
    var inline219 string = t192.vtable.show(t192.data)
    t193 = inline219
    var inline216 string = _goml_m_trait__impl_i_ToString_i_string_i_to__string(t193)
    _goml_runtime_core_string_println(inline216)
    return struct{}{}
}

func _goml_m_trait__impl_i_ToString_i_string_i_to__string(self__67 string) string {
    return self__67
}

func main() {
    main0()
}
