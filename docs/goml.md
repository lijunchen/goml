...
├── test/
│   ├── internal/
│   │   └── pkg/<canonical-package-path>/
│   │       ├── <last-path-segment>.interface
│   │       ├── <last-path-segment>.core
│   │       ├── goml_generated.go
│   │       ├── tests.json
│   │       └── runner
│   └── external/
│       └── pkg/<black-box-test-package-path>/
│           ├── <last-path-segment>.interface
│           ├── <last-path-segment>.core
│           ├── goml_generated.go
│           ├── tests.json
...
The executable file of the module root entry package is `bin/<module name>`; the nested entry package retains the directory within the module and appends the entry name. For example, the output of `alice::app::cmd::server` is `bin/cmd/server/server`.The Go entry, manifest and runner of the test are located in the corresponding `test/internal/pkg/...` or `test/external/pkg/...` package directory. The runner file has the `.exe` suffix on Windows.External dependencies use the same `deps/<owner>/<module>/<version>/pkg/...` structure in the root directory of each stage.
